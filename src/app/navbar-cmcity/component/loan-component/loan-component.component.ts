...
export class LoanComponentComponent implements OnInit {
          ...
          next: (res) => {
            if (res !== null || res) {
              ...
              this.formModelLoanNew.patchValue({
                interestPercent: res.interestPercent
                  ? res.interestPercent + '%'
                  : '5%',
  ...
  exportMakePDFById(mode: any, loanInfo: any[]) {
    ...
    let detailLoan = info.map(function (item) {
      return [
        ...
        {
          text: item.loanYear >= 2567 ? item.loanNo : ' ',
          alignment: 'center',
        },
        {
          text: decimalPipe.transform(item.interestPercent),
        ...
        },
        {
          text:
            item.installment < 0
              ? '-'
              : decimalPipe.transform(item.installment),
          alignment: 'center',
        },
  ...
  checkTotalListGroup(listGroup: any[]) {
    ...
    listGroup?.forEach((element, _index, _array) => {
      ...
      totalValueInterestSum =
        totalValueInterestSum +
        Number(element.totalValueInterest ? element.totalValueInterest : 0);
      outStandInterestSum =
        outStandInterestSum +
        Number(element.outStandInterest ? element.outStandInterest : 0);
      totalValuePrincipleSum =
        totalValuePrincipleSum +
        Number(element.totalValuePrinciple ? element.totalValuePrinciple : 0);
      outStandPrincipleSum =
        outStandPrincipleSum +
        Number(element.outStandPrinciple ? element.outStandPrinciple : 0);
    });
  ...
  exportMakePDFAll(mode: any, listSum: any[]) {
    ...
    let dataSum1 = this.checkListSumAllByDepartment(
      listSum,
      'แขวงเม็งราย',
      this.infogroup1
    );
    let data2 = this.checkListDataPDF(this.infogroup2);
    let dataSum2 = this.checkListSumAllByDepartment(
      listSum,
      'แขวงกาวิละ',
      this.infogroup2
    );
    let data3 = this.checkListDataPDF(this.infogroup3);
    let dataSum3 = this.checkListSumAllByDepartment(
      listSum,
      'แผนงานบริหารทั่วไป',
      this.infogroup3
    );
    let data4 = this.checkListDataPDF(this.infogroup4);
    let dataSum4 = this.checkListSumAllByDepartment(
      listSum,
      'งานเทศกิจ',
      this.infogroup4
    );
    let data5 = this.checkListDataPDF(this.infogroup5);
    let dataSum5 = this.checkListSumAllByDepartment(
      listSum,
      'งานโรงพยาบาล',
      this.infogroup5
    );
    ...
    let dataSum6 = this.checkListSumAllByDepartment(
      listSum,
      'งานก่อสร้าง',
      this.infogroup6
    );
    let data7 = this.checkListDataPDF(this.infogroup7);
    let dataSum7 = this.checkListSumAllByDepartment(
      listSum,
      'งานบริหารงานคลัง',
      this.infogroup7
    );
    let data8 = this.checkListDataPDF(this.infogroup8);
    let dataSum8 = this.checkListSumAllByDepartment(
      listSum,
      'งานบริหารงานคลัง ฝ่ายประจำ',
      this.infogroup8
    );
    let data9 = this.checkListDataPDF(this.infogroup9);
    let dataSum9 = this.checkListSumAllByDepartment(
      listSum,
      'งานบริหารงานทั่วไป',
      this.infogroup9
    );
    let data10 = this.checkListDataPDF(this.infogroup10);
    let dataSum10 = this.checkListSumAllByDepartment(
      listSum,
      'งานบริหารทั่วไป',
      this.infogroup10
    );
    ...
    let dataSum16 = this.checkListSumAllByDepartment(
      listSum,
      'งานบริหารทั่วไปเกี่ยวกับอุตสาหกรรมและการโยธา',
      this.infogroup16
    );
    let data17 = this.checkListDataPDF(this.infogroup17);
    let dataSum17 = this.checkListSumAllByDepartment(
      listSum,
      'งานป้องกันและบรรเทาสาธารณภัย',
      this.infogroup17
    );
    let data18 = this.checkListDataPDF(this.infogroup18);
    let dataSum18 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลดอกเงิน',
      this.infogroup18
    );
    let data19 = this.checkListDataPDF(this.infogroup19);
    let dataSum19 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดเชียงยืน',
      this.infogroup19
    );
    let data20 = this.checkListDataPDF(this.infogroup20);
    let dataSum20 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดกู่คำ',
      this.infogroup20
    );
    ...
    let dataSum21 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดท่าสะต๋อย',
      this.infogroup21
    );
    let data22 = this.checkListDataPDF(this.infogroup22);
    let dataSum22 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดพวกช้าง',
      this.infogroup22
    );
    let data23 = this.checkListDataPDF(this.infogroup23);
    let dataSum23 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดศรีปิงเมือง',
      this.infogroup23
    );
    let data24 = this.checkListDataPDF(this.infogroup24);
    let dataSum24 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดศรีสุพรรณ',
      this.infogroup24
    );
    let data25 = this.checkListDataPDF(this.infogroup25);
    let dataSum25 = this.checkListSumAllByDepartment(
      listSum,
      'งานระดับก่อนวัยเรียนและประถมศึกษา โรงเรียนเทศบาลวัดหมื่นเงินกอง',
      this.infogroup25
    );
